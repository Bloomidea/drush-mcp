...
- CLI flags and `drush-mcp.yml` do not merge per site: `parseCliArgs` returns `null` only when no site flag is given, and only then does `mergeConfig` use the file's `sites:`. A server started with `--docker --host ...` never sees `file_upload` from the YAML. Before 0.4.0 the file was never used at all (and `--config` was ignored).
